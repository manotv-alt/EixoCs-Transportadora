...
int main(){ //em caso de erro, olhe para a variavel 'quantidade_cidades'
    /*
    grafo rotas_cidades;
    cidade *cidades;
    ...
    */
    ...
    rotas_cidades.tamanho_lados_grafo = quantidade_cidades + 1;
    rotas_cidades.matrix_grafo = gerar_grafo(quantidade_cidades + 1, quantidade_cidades + 1);
    ...
    //printar_vetor(rotas_ate_destino_f, registrar_cidades);
...
void preencher_grafo(int tamanho_grafo, grafo matrix, cidade *lista_cidades){ //preciço alterar a parada do --
    ...
    for(int contador = 0; contador < matrix.tamanho_lados_grafo - 1; contador++){
        ...
        printf("%7d %34s\n", lista_cidades[contador].indice_cidade + 1, lista_cidades[contador].nome);
    }
...
void mostrar_grafo(grafo matrix){
    for (int contador = 0; contador < matrix.tamanho_lados_grafo;
         contador++){
        if (contador == 0){
            printf("%5s|", "");
            for (int contador2 = 0;
...
int *regressao(int *vetor_entrada, grafo matrix, int cidade_origem, int cidade_destino){
    ...
    bool *estado_vertices_acessados = (bool*) malloc(matrix.tamanho_lados_grafo*sizeof(bool));
...
cidade *registrar_cidades(int quantidade_cidades){
    ...
    for(int contador = 0; contador < quantidade_cidades; contador++){
        ...
        do{
        printf("Insira um indice valido para essa cidade no grafo: [indice >= 1 e indice <= quantidade cidades, e nao pode ja ter sido usado anteriormente]\n>> ");
        scanf("%d%*c", &(temporaria.indice_cidade));
        temporaria.indice_cidade--;
